...
const PlannerHelper = () =>{
    ...
    const addLimitations = () => {
        ...
        if(currentLimitations.length === 0 || currentLimitations.every((lim) => lim.value.trim() !== '')){
            setConfig((prevConfig) => ({
                ...prevConfig,
                limitations: [...currentLimitations, {id: Date.now(), value: '0'}],
            }));
        }else{
    ...
    const removeLimitations = (id) => {
      ...
      const updateConfigBeforeSubmit = () => {
        setConfig((prevConfig) => {
            const updatedConfig = { ...prevConfig };
            
            shipTypes.forEach((type) => {
                if (!updatedConfig[type.label] || updatedConfig[type.label].trim() === "") {
                    updatedConfig[type.label] = "0";
                }
            });
            updatedConfig.limitations = updatedConfig.limitations.map((lim) => ({
                ...lim,
                value: lim.value.trim() === "" ? "0" : lim.value,
            }));
