   ...
   const { id, name, tier, nation, class: className } = shipData;
    ...
    return ( 
        <div key={id} className="ship-list-item">
            <img src={nationIcon} alt={`${nation} flag`} className="nation-icon" />
            <img src={classImage} alt={`${className} icon`} className="class-icon" />
            <div className="ship-info">
            <div className="ship-name">{name}</div>
            <div className="ship-tier">Tier: {tierLabel}</div>
        </div>
      </div>
    )
};
