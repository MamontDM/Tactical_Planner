...
const ShipTemplate = () => {
        ...
        const handleClearShip = (type, shipIndex) =>{
            setClearListFlag((prev) => ({
                ...prev,
                [type]: true,
            }));
            updateConfigField(type, -1);
        };

        const handleClearAllShipList = () => {
            const updateFlags = Object.keys(config).reduce((acc, type) => {
                if(type !== 'limitations'){
                    acc[type] = true;
                }
                return acc;
            }, {});
            setClearListFlag(updateFlags);
            clearSelectedShips();
        }
