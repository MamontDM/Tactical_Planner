...
const ShipListCard = ({viewType = 'card' }) => {
    
   const {selectedShip , removeShip} = useShipContext();
   console.log(JSON.stringify(selectedShip, null, 2));
   const { id, name, tier, nation, class: className } = selectedShip;
   console.log(selectedShip.name);
        ...
        <>
            <div key={id} className="ship-list-item">
                ...
                <img src={nationIcons[nation]} alt={`${nation} flag`} className="nation-icon" />
                 <button onClick={() => removeShip(selectedShip)}>Remove</button>
            </div>
