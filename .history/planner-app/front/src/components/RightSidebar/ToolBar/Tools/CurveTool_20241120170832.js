...
const CurveTool = ({ isActive }) => {
    ...
    useEffect(() => {
        if (isActive && canvasRef?.current) {
            ...
            virtualCanvas.width = ctx.width;
            virtualCanvas.height = ctx.height;
            const virtualCtx = virtualCanvas.getContext('2d');


            const redrawVitrualCanvas = () => {
            ...
            const handleMouseUp = () => {
                ...
                if (points.current.length > 1) {
                    ...
                    curveEndDrawArrow(ctx, fromX, fromY, toX, toY, angle, headLength, lineWidth, strokeStyle);
