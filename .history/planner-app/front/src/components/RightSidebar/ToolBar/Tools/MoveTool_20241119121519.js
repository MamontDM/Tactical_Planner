...
const MoveTool = ({isActive}) => {
 ...
 const handleMouseMove = (event) => {
    if (isDragging && selectedObject) {
        const { x, y } = getCoordinates(event, canvasRef.current);
        const dx = selectedObject.points[0].x - initialX;
        const dy = selectedObject.points[0].y - initialY;

        selectedObject.points[0].x = x - offsetX;
        selectedObject.points[0].y = y - offsetY;
