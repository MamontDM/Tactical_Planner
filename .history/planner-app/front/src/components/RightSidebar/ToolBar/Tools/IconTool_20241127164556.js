    ...
    useEffect(() => {
        ...
        if (isActive && canvasRef?.current && drawingCanvasRef?.current) {
            ...
            const handleMouseMove = (event) => {
                ...
                rotationAngle.current = Math.atan2(y - startX)
                clearDrawingCanvas();
                redrawMainCanvas();

                drawTemporaryIcon(drawingCtx,  x, y, shipImg.current, rotationAngle, fillColor.current,  shipLabel.current );
            };
