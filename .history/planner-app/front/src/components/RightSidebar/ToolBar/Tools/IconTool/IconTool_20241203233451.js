...
const IconTool = ({isActive, settings}) =>{ 
      ...
      useEffect(() => {
        if (settings) {
            ...
            if (settings.fillColor) {
                fillColor.current = settings.fillColor;
            }
            if (settings.shipLabel) {
                shipLabel.current = settings.shipLabel;
            }

            console.log(shipLabel.current, shipType.current, fillColor.current)
            generateImage();
        }
    }, [settings]);
    ...
    useEffect(() => {
        ...
        getDrawingCanvasContext, clearDrawingCanvas, img, settings
    ]);
