...
const IconTool = ({isActive, settings}) =>{ 
    ...
    const handleSetColor = (color) => {
      ...
      useEffect(() => {
        if (settings) {
            if(settings.shipType && settings.fillColor && settings.shipLabel){
                handleSetShipIcon(settings.shipType);
                handleSetColor(settings.fillColor);
                handleSetLabel(settings.shipLabel);
            }
        }
    }, [settings]);
