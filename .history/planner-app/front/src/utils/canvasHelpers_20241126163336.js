...
export const drawTemporaryIcon = (ctx, { x, y, key, color, label }) => {
    const icon = getIconByKey(key); // Получаем иконку по ключу (например, SVG-данные)
    
    if (!icon) return;

    // Отрисовка SVG иконки (например, с помощью Canvas API)
    ctx.save();
    ctx.fillStyle = color;
    ctx.translate(x, y);

    // Отрисовка примитивной иконки (замените на реальный процесс отрисовки)
    ctx.beginPath();
    ctx.arc(0, 0, 20, 0, Math.PI * 2); // Пример: круг
    ctx.fill();
