...
export function findClickedObject(clickX, clickY, objects) {
    ...
    switch (object.type){
          ...
          case "radar": 
            ...
            case "vision": 
             ...
             case "icon":
              const tolerance = 15;
                if(isPointOnIcon(object, clickX, clickY, tolerance)){
                  return object;
                };
            break;
            case "text":
                if(isPointOnText(object, clickX, clickY)){
                  return object;
                }
            break;
  ...
  function isPointOnIcon(object, clickX, clickY, tolerance){
    if(object.type === 'icon'){
  ...
  function isPointOnText(object, clickX, clickY){
    console.log(clickX, clickY);
    console.log(object.fontSize);
    const diapazoneRight = object.x + (object.width/2);
    const diapazoneLeft = object.x - (object.width/2);
    const diapazoneTop = object.y + (object.fontSize/2);
    const diapazoneBottom = object.y - (object.fontSize/2);

console.log(`diapazoneRight is ${diapazoneRight}, diapazoneLeft is : ${diapazoneLeft}`);
console.log(`diapazoneTop is ${diapazoneTop}, diapazoneBot is : ${diapazoneBottom}`);
    
    if(
      clickX <= diapazoneRight &&
      clickX >= diapazoneLeft && 
      clickY >= diapazoneTop && 
      clickY <= diapazoneBottom)
      {
        return object; 
    }else{
      return console.log('miss click');
      }
  };
