...
const Toolbar = () =>  {
    ...
    const handleToolClick = (id) => {
        ...
        console.log(`activated new:  ${id}`);
    };

    useEffect(() => {
        if (activeToolId) {
            activateToolLogic(activeToolId);
        }
    }, [activeToolId]);

    const activateToolLogic = (toolId) => {
        const activate = toolActivationLogic[toolId];
        if (activate) {
            activate();
        } else {
