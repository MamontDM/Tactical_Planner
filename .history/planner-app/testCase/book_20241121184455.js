...
class Library {
    ...
    returnBook(bookId, student){
        const index = this.bookStorage.findIndex(item => item === bookId);
        if(index !== -1){
            this.bookStorage.splice(index, 1 , bookId );
        }else{
            console.log('!');
        }
        return this.bookStorage.splice()
    }
...
const returnBook = lib.returnBook('Pyok', 'Onn');
