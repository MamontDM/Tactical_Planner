...
const router = express.Router(); // Создаем объект маршрутов
...
// Обработка данных кораблей
function processShipData(apiData) {
    console.log('Данные кораблей:', JSON.stringify(apiData, null, 2));
    return apiData; // Здесь можно добавить логику обработки
}

// Маршрут для получения данных о кораблях
router.get('/', async (req, res) => {
    try {
        const shipData = await getShipData();
        res.json(shipData); // Отправляем данные клиенту
    } catch (error) {
